# -*- coding: utf-8 -*-
"""FloorPlan-v0.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1GbvjNYlN0LtvM_0DnFYTcid98rWbtZXv
"""

import gym
from gym import spaces
import random
import glob
from PIL import Image
import pygame
import numpy as np
import matplotlib.pyplot as plt
from IPython import display as ipythondisplay
from matplotlib.pyplot import figure, draw, pause
from skimage.transform import resize

##### CONSTANTS ######


cmap = {1: {0: [[255, 255, 255], '#FFFFFF'], 127: [[0, 0, 0], '#000000'], 255: [[255, 0, 0], '#FF0000'],
            -1: [[192, 192, 192], '#C0C0C0']},
        2: {0: [[210, 105, 30], '#D2691E'], 1: [[220, 20, 60], '#DC143C'], 2: [[255, 215, 0], '#FFD700'],
            3: [[30, 144, 255], '#1E90FF'], 4: [[244, 164, 96], '#F4A460'], 5: [[255, 99, 71], '#FF6347'],
            6: [[240, 128, 128], '#F08080'], 7: [[255, 160, 122], '#FFA07A'], 8: [[255, 165, 0], '#FFA500'],
            9: [[0, 255, 127], '#00FF7F'], 10: [[255, 228, 181], '#FFE4B5'], 11: [[188, 143, 143], '#BC8F8F'],
            12: [[139, 69, 19], '#8B4513'], 13: [[255, 255, 255], '#FFFFFF'], 14: [[0, 0, 0], '#000000'],
            15: [[255, 0, 0], '#FF0000'], 16: [[128, 128, 128], '#808080'], 17: [[128, 0, 0], '#800000'],
            -1: [[192, 192, 192], '#C0C0C0']},
        4: {0: [[255, 255, 255], '#FFFFFF'], 255: [[119, 136, 153], '#778899'], -1: [[192, 192, 192], '#C0C0C0']}}

######################


class FloorPlanEnv(gym.Env):
  """Custom Environment that follows gym interface"""
  metadata = {"render_modes": ["human", "rgb_array"], "render_fps": 10}

  def __init__(self, data_dir=None):
    super(FloorPlanEnv, self).__init__()
    
    self.window_size = 512
    self.window = None
    self.clock = None

    self.perimetre = 0 #TODO: info?
    self.num_rooms = 8 #TODO: info?

    fixed_rooms = [0, 1, 2, 3]
    optional_rooms = np.random.randint(3, 12, size=self.num_rooms-4)
    total_rooms = fixed_rooms + list(optional_rooms)
    self.rooms_type = dict(zip(range(self.num_rooms), total_rooms))

    self.bounds = {
        "pos": (0, 255),
        "size": (1, 10),
        "proportion": (0.5, 10)
    }

    obs_space = dict()
    for room in range(self.num_rooms):
      space = spaces.Dict({
            "pos": spaces.Box(low=0, high=255, shape=(2,), dtype=np.uint8),
            "size": spaces.Discrete(5),
            "proportion": spaces.Box(low=0.1, high=10, shape=(1,), dtype=np.float16)
      })
      obs_space.update({room: space})
    
    # Define observation space
    self.observation_space = spaces.Dict(obs_space)

    # Define action space

    self._actions = {
              0: ("pos", [5, 0]),
              1: ("pos", [-5, 0]),
              2: ("pos", [0, 5]),
              3: ("pos", [0, -5]),
              4: ("size", 1),
              5: ("size", -1),
              6: ("proportion", 0.5),
              7: ("proportion", -0.5),
    }

    self.action_space = spaces.Box(low=np.array([0,0]), high=np.array([self.num_rooms-1, len(self._actions)-1]), dtype=np.uint8)
    
    
    self.floors = self._load_floors(data_dir)
    self.floor = next(self.floors)

  def _load_floors(self, data_dir):
    # Load the entire dataset as a generator
    
    files = glob.glob(data_dir + '/*.png')

    for file in files:
      im = Image.open(file) #rgba image
      im_array = np.asarray(im)
      yield(im_array)
  
  def _distance_to_door(self, room):
    pos_door = self.floor[0]
    pos_room = self.current_state[room]["pos"]
    euclidean = lambda x, y: np.linalg.norm(x-y)
    return euclidean(pos_room, pos_door)

  def reset(self, seed=None, return_info=False):
      # Reset the state of the environment to an initial state
      # self.floor = next(self.floors)
      self.current_state = dict()
      for room in range(self.num_rooms):
        room_state = {
          "type": self.rooms_type[room], # TODO: better in info
          "pos": np.random.randint(0, 255, size=2),
          "size": np.random.randint(1, 5),
          "proportion": 1,
          # "dist_to_door": self._distance_to_door(room) #TODO: better in info
        }
        self.current_state.update({room: room_state})

      obs = self._get_obs()
      return obs

  def step(self, action):
    # Execute one time step within the environment
    self._take_action(action)

    reward = 0
    done = False
    info = {}
    obs = self._get_obs()
    return obs, reward, done, info

  def _inside_perimeter(self, prev_value, value):
    raw = self.floor[:,:,0]
    rows, cols = [], []

    for i in range(256):
      row_points = list(np.where(raw[i,:] > 0)[0])
      col_points = list(np.where(raw[:,i] > 0)[0])

      row_interval = row_points
      col_interval = col_points

      if row_points:
        row_interval = row_points[:1]+row_points[-1:]
      rows.append(row_interval) # append not indexing

      if col_points:
        col_interval = col_points[:1]+col_points[-1:]
      cols.append(col_interval)
    
    pos = prev_value + value
    print(pos)


  def _take_action(self, action):
    # Make changes to our environment
    room, num_action = action

    state_variable, value = self._actions[num_action]
    prev_value = self.current_state[room][state_variable]
    # if state_variable == "pos":
    #  self.current_state[room]["pos"] = self._inside_perimeter(prev_value, value)
    # else:
    low, high = self.bounds[state_variable]
    self.current_state[room][state_variable] = np.clip(prev_value+value, low, high)


  def _get_obs(self):
    # Return the next observation
    obs = self.current_state
    return obs
    
  def render(self, mode='human', close=False):
    # Render the environment to the screen

    canvas_factor = self.window_size/256
    # canvas_factor = 1
    if self.window is None and mode == "human":
      
      # initialize
      pygame.init()
      pygame.display.init()
      self.window = pygame.display.set_mode((self.window_size, self.window_size))

    if self.clock is None and mode == "human":
        self.clock = pygame.time.Clock()

    canvas = pygame.Surface((self.window_size, self.window_size))
    canvas.fill((255, 255, 255))

    raw = (self.floor[:,:,0]-255)*-1
    img = np.ones((256, 256, 3), dtype=np.uint8)*255
    img[:,:,0] = raw
    img[:,:,1] = raw
    img[:,:,2] = raw

    if self.window_size > 256:
      scale = self.window_size//256
      scaleup = np.zeros((self.window_size,)*2 +(3,))
      scaleup[::scale,::scale,:] = img
      scaleup[1::scale,::scale,:] = img
      scaleup[:,1::scale] = scaleup[:,::scale]
      img = scaleup

    img = pygame.surfarray.make_surface(img)
    canvas.blit(img, (0, 0))
    
    center_square_size = 3*canvas_factor # The size of the center point in pixels

    
    for room in range(self.num_rooms):
      coord = self.current_state[room]["pos"]*canvas_factor
      area = (self.current_state[room]["size"]+1)*200
      prop = self.current_state[room]["proportion"]

      color = cmap[2][room][0]

      # Now we draw the room
      
      # Entire room rectangle
      #   _____________
      #  |             |
      # w|      c      |
      #  |_____________|
      #         l
      #
      # length and width
      #  area = l*w
      #  prop = l/w
      #  l = srqt(area*prop)

      l = np.sqrt(area*prop) * canvas_factor
      w = area/l * canvas_factor
      room_coord = coord + [-l//2, -w//2]
      pygame.draw.rect(
          canvas,
          tuple(color),
          pygame.Rect(
              room_coord,
              (l, w),
          ),
          3
      )

      # Room center point
      center_coord = coord - [center_square_size//2]*2

      pygame.draw.rect(
          canvas,
          color,
          pygame.Rect(
              center_coord,
              (center_square_size, center_square_size),
          ),
      )
      

    if mode == "human":
        # The following line copies our drawings from `canvas` to the visible window
        self.window.blit(canvas, canvas.get_rect())
        pygame.event.pump()
        pygame.display.update()

        # We need to ensure that human-rendering occurs at the predefined framerate.
        # The following line will automatically add a delay to keep the framerate stable.
        self.clock.tick(self.metadata["render_fps"])
    else:  # rgb_array
        return np.transpose(
            np.array(pygame.surfarray.pixels3d(canvas)), axes=(1, 0, 2)
        )

  def close(self):
    if self.window is not None:
            pygame.display.quit()
            pygame.quit()





def test_env():
  env = FloorPlanEnv("./data")
  env.reset()
  env.render()

  # prev_screen = env.render(mode='rgb_array')
  # fg = figure()
  # ax = fg.gca()
  # visualizer = ax.imshow(prev_screen)

  for i in range(1000):
    action = env.action_space.sample()
    obs, reward, done, info = env.step(action)
    #if i%5 == 0:
        # screen = env.render(mode='rgb_array')
        # visualizer.set_data(screen)
        # draw(), pause(1e-3)
        # Other option, just use: env.render() # mode=human by default
    env.render()
    if done:
      break
  env.close()

def testing():
  env = FloorPlanEnv("./data")
  env.reset()
  env.render()

  for i in range(1000):
    action = env.action_space.sample()
    obs, reward, done, info = env.step(action)
    env.render()
    if done:
      break
  env.close()


def test():
  env = FloorPlanEnv("./data")
  env.reset()
  prev_screen = env.render(mode='rgb_array')
  fg = figure()
  ax = fg.gca()
  visualizer = ax.imshow(prev_screen)
  draw(), pause(2)

def main():
  testing()














if __name__ == '__main__':
  main()


"""
# Using stable_baselines from openai
from stable_baselines.common.policies import MlpPolicy
from stable_baselines import PPO2

env = CustomEnv('/data')

# Training
model = PPO2(MlpPolicy, env, verbose=1)
model.learn(total_timesteps=20000)

# Testing
n_steps = 2000  # maximum number of steps per episode
episode_return = 0
obs = env.reset()
for i in range(n_steps):
  action, _states = model.predict(obs)
  obs, rewards, done, info = env.step(action)
  if done:
    break
  env.render()
  episode_return += rewards
print("Episode return:", episode_return)
"""
